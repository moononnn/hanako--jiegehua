...
export function loadProviderConfigs() {
  try {
    ...
    console.error("[接个话] 读取供应商配置失败:", e.message);
    return {};
  }
}
...
export function loadModelsCatalog() {
  try {
    ...
    console.error("[接个话] models.json 读取失败:", e.message);
    return { providers: {} };
  }
}
...
// 兼容两种输出：对象数组 [{text, direction}] 与字符串数组（旧格式，direction 置空）
export function parseSuggestions(raw, count) {
  ...
  return lines.map((t) => ({ text: t, direction: "" })).slice(0, count);
}
